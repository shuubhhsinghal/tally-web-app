...
def call_metadata_extraction(images: list[bytes], is_retry: bool = False) -> dict:
    """Extracts metadata (supplier, invoice number, date, taxes, printed totals) from the un-cropped images."""
    ...
    try:
        ...
        Extract the Supplier Name, Invoice Number, Date, and Taxes.
        ...
        6. printed_subtotal: Look for a label like "Sub Total", "Taxable Total", "Taxable Value",
           or "Total Taxable Amount" — the sum of all line item amounts BEFORE GST is added.
           Return null if not clearly present on the invoice.
