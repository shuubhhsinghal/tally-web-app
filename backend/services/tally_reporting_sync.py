...
async def fetch_and_store_cost_centres():
  ...
  <BODY>
    ...
    try:
        response = await tally_transport.post(payload, timeout=30)
        ...
        # Off the event loop: a blocking sqlite3 write for every cost centre,
        # run inline here would freeze every concurrent request (and the
        # background sync loop shares this same event loop) for however long
        # the write takes.
        await run_in_threadpool(_store_cost_centres, cost_centres)
...
def _store_cost_centres(cost_centres: list):
    with get_db() as conn:
        cursor = conn.cursor()
...
async def fetch_and_store_vouchers(start_date: str, end_date: str):
  ...
  <BODY>
    ...
    try:
        response = await tally_transport.post(payload, timeout=120)
        response.raise_for_status()

        # Off the event loop: parsing a fiscal year of vouchers and writing
        # every ledger/inventory/cost-centre row one statement at a time is
        # pure synchronous CPU+SQLite work with no natural await point. Left
        # inline, it would freeze every concurrently-served request (and the
        # background sync loop shares this same event loop) for however long
        # this takes -- seconds to tens of seconds for a real fiscal year.
        return await run_in_threadpool(_process_and_store_vouchers, response.text, start_date, end_date)
    except Exception as e:
        print(f"Error fetching reporting vouchers: {e}")
        raise e

def _process_and_store_vouchers(response_text: str, start_date: str, end_date: str) -> int:
    safe_xml_text = sanitize_tally_xml(response_text)
    root = ET.fromstring(safe_xml_text)
...
async def fetch_live_purchase_history_from_tally(item_name: str, since_date: str) -> list:
  ...
  <BODY>
    ...
    response = await tally_transport.post(payload, timeout=10)
    response.raise_for_status()
